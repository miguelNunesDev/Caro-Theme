...
<section class="w-full mx-auto text-white" id="case-study-list" <?= get_block_wrapper_attributes(); ?>>
    <article class="mt-[9rem] flex flex-wrap">
        ...
        <ul class="md:overflow-y-scroll md:basis-1/2 md:h-[60rem]">
            ...
            foreach ($attributes['cases'] as $id) : ?>
                ...
                <li class="relative p-[3.2rem] border-white border-t <?= $i ? '' : 'md:border-t-0' ?>" data-id="<?= $id ?>">
                    <a class="max-sm:p-[3.2rem] max-sm:flex max-sm:justify-between max-sm:items-center" href="<?= get_the_permalink($id) ?>">
                        <span class="max-sm:max-w-[25.4rem]">
                            ...
                            <p class="pl-[.9rem] font-open-sans text-[1.8rem] md:text-[2rem] leading-[1.2em] font-light"><?= get_the_title($id) ?></p>

...
class ContactBlock {
  ...
  initRotator() {
    ...
    setInterval(() => {
      this.resetRotators();
      this.setRotator(this.rotators[this.current]);
      this.current = this.current >= this.rotators.length - 1 ? 0 : ++this.current;
  ...
  setRotator = (rotator, state = true) => {
    ...
    this.rotatorWrapper.style.width = rotator.getBoundingClientRect().width + 'px';
  };
  resetRotators = () => {
...
const heroBanner = new ContactBlock('#hero-banner');
/******/ })()
;
